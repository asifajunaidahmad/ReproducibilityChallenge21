...
class emotional_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = '/content/drive/MyDrive/fake_flow_small_dataset/processed_files/features/emotional_features_{}_{}.npy'.format(self.model_name, self.representation)
        if exists(file_name):
...
class sentiment_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = '/content/drive/MyDrive/fake_flow_small_dataset/processed_files/features/sentiment_features_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
...
class morality_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = '/content/drive/MyDrive/fake_flow_small_dataset/processed_files/features/morality_features_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
...
class imageability_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = '/content/drive/MyDrive/fake_flow_small_dataset/processed_files/features/imageability_features_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
...
class hyperbolic_features(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        file_name = '/content/drive/MyDrive/fake_flow_small_dataset/processed_files/features/hyperbolic_features_{}.npy'.format(self.model_name)
        if exists(file_name):
            features = np.load(file_name).tolist()
        else:
            data = Parallel(n_jobs=self.n_jobs, backend="multiprocessing", prefer="processes") \
