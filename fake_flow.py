...
class fake_flow:
    ...
    def prep_embed(self):
        path = '/content/drive/MyDrive/fake_flow_small_dataset/features'
    ...
    def evaluate_on_test(self):
        ...
        plt.savefig('/content/drive/MyDrive/fake_flow_small_dataset/processed_files/confusion_matrix.png')
        print(classification_report(Y_test, Y_test_pred))

    def run_model(self, type_='train'):
        file = '/content/drive/MyDrive/fake_flow_small_dataset/processed_files/savemodel/{}_{}.h5'.format(self.__class__.__name__, self.model_name)
        # 1, Compile the model
        ...
        if type_ == 'train':
            self.model.fit(x=[self.train['features'], self.train['text']], y=self.train['label'], batch_size=self.parameters['batch_size'], epochs=self.parameters['max_epoch'], verbose=self.verbose,
                           validation_data=([self.dev['features'], self.dev['text']], self.dev['label']))
    ...
    def objective_function(self, params):
        ...
        df_summary_table.to_csv('/content/drive/MyDrive/fake_flow_small_dataset/processed_file/savemodel/results_{}_{}.csv'.format(self.__class__.__name__, self.model_name), header=True, index=False)
