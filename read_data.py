...
def prepare_input(dataset='MultiSourceFake', segments_number=10, n_jobs=-1, emo_rep='frequency', return_features=True,
    ...
    content = pd.read_csv('/content/drive/MyDrive/fake_flow_small_dataset/data/{}/shuffle_data.csv'.format(dataset))
    content_features = []
    """Extract features, segment text, clean it."""
    if return_features:
        content_features = manual_features(n_jobs=n_jobs, path='/content/drive/MyDrive/fake_flow_small_dataset/features', model_name=dataset,
